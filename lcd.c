...
void LCD_WR_Queue(){
    if (r!=w) {                                     // Buffer empty?
       if (spi_i2s_flag_get(SPI0,SPI_FLAG_TBE)) {   // ...no! Device redy?
          OLED_CS_Clr();                            // ......Yes! CS (again)
          (queue[r]>=1<<8) ? OLED_DC_Set() : OLED_DC_Clr(); //    DC
          spi_i2s_data_transmit(SPI0, queue[r++]&0xFF); //        Write!
          r%=256;                                   //            Advance.
        }                                           //       (No! Return!)
    } else {
...
void Lcd_Init(void)
{
	...
	gpio_bit_set(GPIOB, GPIO_PIN_2);   // CS = 1 (inactive)
	gpio_bit_set(GPIOB, GPIO_PIN_0);   // DC = 1 (data default)
